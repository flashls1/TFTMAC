...
`DeviceProfiles.ini` contains17 unique target CVars. Generated Saved config uses repeated **CVars=** assignments and **DeviceProfileFragment** sections. The target is the game's private `files/UnrealGame/TFT/TFT/Saved/Config/Android/DeviceProfiles.ini`. The native transaction verifies the file hash, root ownership,0444 mode, app SELinux context and zygote mount namespace before unrooting. A fresh engine log must confirm every target before readiness is claimed.
...
Boot acceptance is **not** proof of sustained60FPS. Use the existing native gameplay recorder and combat benchmark for that claim. Current UE logs identify OpenGL through ANGLE; historical direct-Vulkan documentation does not describe this boot.

The existing native DEV launch also selects Android HWUI `skiagl` before the game starts. Fresh2026-09-05 ANR traces matched the documented WebView Vulkan image-view destruction deadlock. This property only selects Android UI rendering; TFT retains ANGLE/Vulkan. DEV reports ANR dialogs without automatically clicking Close app.
