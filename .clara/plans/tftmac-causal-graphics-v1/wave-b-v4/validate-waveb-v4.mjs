...
function fail(message) { throw new Error(`WAVE_B_V4_VALIDATION_FAILED: ${message}`); }
...
for (const required of [
  `${waveRoot}/CORRECTION_AMENDMENT.md`,
...
for (const required of [
  ...
  'configuration.authority.resolveForLaunch',
...
const testCount = fs.readdirSync(absolute('Tests/TFTMACTests'))
  ...
  .reduce((total, name) => {
    const source = readText(`Tests/TFTMACTests/${name}`);
    return total + (source.match(/^    func test/gm) ?? []).length;
  }, 0);
...
assert(receipt.state === 'WAVE_B_V4_IMPLEMENTATION_APPLIED', 'implementation receipt state drifted');
...
console.log(JSON.stringify({
  state: 'WAVE_B_V4_SOURCE_VALIDATION_PASS',
