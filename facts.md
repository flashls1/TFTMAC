...
**Current DEV evidence through:** 2026-09-11 live package/update-authority audit plus current optimization results; latest performance winner remains `DEV-B8-WIN-01`. Live audit observed `18.1-5423749` / `8423749` installed with `installer=null` / shell initiation while a newer official Riot/Google Play client is available; this is an update-authority violation to remediate, not a new performance winner.
...
**LOCKED LIVE-CLIENT UPDATE AUTHORITY:** the playable DEV path must never treat a merely-present Riot package as current. Google Play (`com.android.vending`) is the install/update authority for `com.riotgames.league.teamfighttactics`; no specific Riot version is permanently pinned. Before a session can be labeled ready for real matchmaking or admitted to performance tuning, the runtime must verify the production package identity, verify Google Play installer ownership, surface the official Play listing, and refuse `MATCH_READY`/equivalent readiness while Play reports Install/Update or package authority is missing/wrong. PBE remains excluded.

**LOCKED RIOT UPDATE AUDIT / DIFF POLICY:** every observed Riot/TFT client update must be treated as a measurable runtime transition. Before the update, capture package version/code, installer source, package/split paths and hashes, a complete file inventory/hash manifest for readable Riot package payload and app-owned content while TFT is stopped, and the current graphics/runtime fingerprint. After Google Play delivery, repeat the inventory before first TFT launch; after Riot's own first-launch content initialization settles, capture a third inventory. Preserve machine-readable `PRE_UPDATE -> POST_PLAY_UPDATE`, `POST_PLAY_UPDATE -> POST_RIOT_INIT`, and overall diffs with every added/removed/modified file, old/new hash and size where readable. Separately compare engine/native-library/graphics-pipeline identities and run a matched gameplay baseline on the new client before resuming optimization so update-driven gains or regressions are not misattributed to TFTMAC tuning.
...
| Current observed installed TFT build | `18.1-5423749`, version code `8423749`; 2026-09-11 audit found shell-installed ownership (`installer=null`) and a newer official client available, so this installed state is not update-authority compliant and must be remediated through Google Play before further performance conclusions |
